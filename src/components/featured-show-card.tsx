...
/** Twice the compact `ShowCard`, which is what makes this row read as the lead. */
export const FEATURED_CARD_WIDTH = SHOW_CARD_WIDTH * 2;
...
export function FeaturedShowCard({ show }: FeaturedShowCardProps) {
  ...
  // Built here rather than in the StyleSheet because both read `theme.scrim`
  // — keeping them off a hardcoded black is what lets the scrim's color be
  // changed in `theme.ts` alone, like every other color in the app.
  const scrimColors = [
  ...
  return (
    <Pressable
      ...
      accessibilityLabel={[show.name, show.theater.name, next && formatShowtime(next.startsAt)]
        .filter(Boolean)
        .join(', ')}
      ...
      <View style={styles.text} pointerEvents="none">
        {show.categories[0] ? (
          <ThemedText type="small" style={[styles.line, styles.contextLine, onImageText]} numberOfLines={1}>
            {show.categories[0]}
          </ThemedText>
        ) : null}

        <ThemedText type="smallBold" style={[styles.line, styles.title, onImageText]} numberOfLines={2}>
          {show.name}
        </ThemedText>

        {next ? (
          <ThemedText type="small" style={[styles.line, onImageText]} numberOfLines={1}>
